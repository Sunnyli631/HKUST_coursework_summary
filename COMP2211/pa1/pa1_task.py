...
Original file is located at
    https://colab.research.google.com/drive/1Hib2xsth4WeKsqpg1I3dRn0XTamEQm38
...
def manhattan_distance(X_train, X_test):
  ...
  # todo start #
  distance = np.transpose(np.sum(np.abs(X_train[:, np.newaxis,:] - X_test[np.newaxis,:,:]), axis=2))
...
def cosine_distance(X_train, X_test):
  ...
  # X_test: numpy array of shape (num_rows_test, num_features)
  # todo start #
  upper = np.dot(X_test, X_train.T)
  lower = np.outer(np.sqrt(np.sum(np.square(X_test), axis=1)), np.sqrt(np.sum(np.square(X_train), axis=1)))
...
def knn_prediction(distances, y_train, k):
  # distances: numpy array of shape (num_rows_test, num_rows_train), return value from previous distance functions
  # y_train: numpy array of shape (num_rows_train, 1),  the labels of training data
  # k: integer, k in "K-nearest neighbors"

  # todo start #
  num_rows_test = distances.shape[0]
  y_classes = np.arange(np.min(y_train), np.max(y_train) + 1)
  prediction = np.zeros(num_rows_test)
  prediction = prediction.astype(np.int64)
  #print(y_classes.shape)
  for x in range(num_rows_test):
    nearest_indices = np.argsort(distances[x])[:k]
    nearest_labels = y_train[nearest_indices]
    nearest_labels = nearest_labels.flatten()
    #np.reshape(nearest_labels, (k,))
    #print(nearest_labels.shape)
    unique_label, counts = np.unique(nearest_labels, return_counts=True)
    uni_length = len(unique_label)
    #zero_indices = np.where(distances[x,:]==0)
    #count = (counts == np.max(counts))
    if (np.count_nonzero(counts == np.max(counts)) > 1):
      count_counts = []
      count_length = len(counts)
      for i in range(count_length):
        if(counts[i]==np.max(counts)):
          count_counts.append(i)
      inverse_distances = 1/(distances[x,nearest_indices]+1e-30)
        #print(inverse_distances.shape)
      weight = np.zeros(count_length)
      for j in count_counts:
        weight[j] = np.sum(inverse_distances[nearest_labels == (unique_label[j])])
        #length wrong?
      prediction[x] = unique_label[np.argmax(weight)]
    else:
      prediction[x] = unique_label[np.argmax(counts)]
  #print(prediction.dtype)
  #prediction = prediction.astype(np.int16)
  # todo end #
  #not finished, need to deal with inverse distance!!!!!!!
...
def f_score(y_test, prediction, y_classes):
  ...
  for i in range(num_classes):
      ...
      true_positive = np.sum((y_test==y_classes[i])&(prediction==y_classes[i]))
      false_positive = np.sum((y_test!=y_classes[i])&(prediction==y_classes[i]))
      false_negative = np.sum((y_test==y_classes[i])&(prediction!=y_classes[i]))
...
def centroid_euclidean_distance(X_train, centroids):
  ...
  # todo start #
  distance = np.sqrt(np.sum(np.square(X_train[:,np.newaxis,:] - centroids[np.newaxis,:,:]), axis=2))
  distance = np.transpose(distance)
  #print(X_train.shape)
...
def calculate_centroids(X_train, assignment, k):
  ...
  for i in range(k):
    cluster_points = X_train[np.where(assignment == i)]
...
def k_means_cluster_reassignment(X_train, initial_centroids, max_iterations=100):
  ...
  for iteration in range(max_iterations):
    ...
    # todo start #
    if (np.array_equal(assignment, prev_ass)):
      break
    prev_ass = assignment
    # todo end #
...
def k_means_centroid_value(X_train, initial_centroids, max_iterations=100, threshold_value=0.0001):
  ...
  for iteration in range(max_iterations):
    ...
    centroids = calculate_centroids(X_train, assignment, k)
    # todo start #
    if (np.max(np.abs(centroids - prev_centroids)) < threshold_value):
      break
    prev_centroids = centroids
    # todo end #
  return assignment, centroids
