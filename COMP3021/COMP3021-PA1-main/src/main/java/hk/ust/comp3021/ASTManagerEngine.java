...
public class ASTManagerEngine {
    ...
    */
    ...
    public HashMap<String, Integer> calculateOp2Nums() {
        ...
        ArrayList<ASTModule> temp2= new ArrayList<>(id2ASTModules.values());
        for(ASTModule temp: temp2){
            ArrayList<ASTElement> nodes = temp.getAllNodes();
            for(ASTElement temp3:nodes){
                if(temp3 instanceof ASTEnumOp op){
                    String opName = op.getOperatorName();
                    if(temp1.containsKey(opName)){
                        int count = temp1.get(opName);
                        temp1.put(opName, count + 1);
                    }else{
                        temp1.put(opName,1);
                    }
                }
    ...
    public String mostCommonUsedOp(HashMap<String, Integer> op2Num) {
        ...
        //HashMap<String,Integer> opFreq = calculateOp2Nums();
        int maxCount=0;
    ...
    public HashMap<String, Set<String>> calculateCalledFunc() {
        ...
        for (ASTModule module : id2ASTModules.values()) {
            for (FunctionDefStmt func : module.getAllFunctions()) {
                ...
                ArrayList<CallExpr> call = func.getAllCalledFunc();
                for (CallExpr node : call) {
                    String calledFuncName = node.getCalledFuncName();
                    calledFuncs.add(calledFuncName);
                }
                func2CalledFuncs.put(func.getName(), calledFuncs);
            }
        }
