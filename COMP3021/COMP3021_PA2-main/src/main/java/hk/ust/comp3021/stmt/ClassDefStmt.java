...
public class ClassDefStmt extends ASTStmt {
    ...
    public ArrayList<FunctionDefStmt> findAllDirMethods(){
        ArrayList<ASTElement> child = getChildren();
        ArrayList<FunctionDefStmt> result = new ArrayList<>();
        for(ASTElement astElement:child) {
            if(astElement instanceof FunctionDefStmt){
                result.add((FunctionDefStmt) astElement);
            }
        }
        return result;
    }
