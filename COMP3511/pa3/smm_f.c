...
void *mm_malloc(size_t size)
{
    ...
    struct meta_data *i=head->next;
    //struct meta_data *last=i;
    while(i!=head){
        if(i->size>=size&&i->free=='f'){
            if(i->size>=size+meta_data_size){
                current_address=(struct meta_data *)((size_t)i+size+meta_data_size);
                struct meta_data *newptr=(struct meta_data *)(current_address);
                newptr->size=i->size-size-meta_data_size;
                newptr->free='f';
                i->size=size;
                i->free='o';
            
                list_add(newptr,i,i->next);
                current_address=(void*)((size_t)i+meta_data_size);
    ...
    current_address=sbrk(size+meta_data_size);
    //current_address=sbrk(0)+meta_data_size;
    struct meta_data *newptr=(struct meta_data *)current_address;
    newptr->size=size;
    newptr->free='o';
    list_add_tail(newptr,head);
    
    current_address=(void*)((size_t)current_address+meta_data_size);
    return current_address;
    //return NULL;
}
