...
int check_timeout(long long now, struct rdt3_sender_ctx *ctx_head, int sockfd) {
    ...
    while (current != NULL) {
		...
		if(current->waiting_ack==1){
			if(now - current->clock>=TIMEOUT){
				struct sockaddr_in clientaddr; //need to check/correct
				memset(&clientaddr, 0, sizeof(clientaddr));
				
				clientaddr.sin_family = AF_INET;
				clientaddr.sin_addr.s_addr = htonl(current->ip);
				clientaddr.sin_port = htons(current->port);
				//sendto(sockfd, current->packet, current->packet_len, 0, (struct sockaddr*)&(current->dest_addr), sizeof(current->dest_addr));
	...
	while (1) {
		...
		if (strncmp(buffer + parse_idx, UPDATE, strlen(UPDATE)) == 0) {
			...
			if (update_node == NULL) {
				 ...
				 * START YOUR CODE HERE
				 **********************************************/
				update_node->file_map = new_map; //update
