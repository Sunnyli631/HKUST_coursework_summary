...
int rdt3_send(int sockfd, struct sockaddr_in servaddr, char ack_num, char *buffer, unsigned len) {
	...
	while (waiting) {
        ...
        //recvfrom(sockfd, (char *)recv_buf, MAXMSG,
                //0, (struct sockaddr *)&recv_addr, &recv_len);

        //seq = recv_buf[0];
        //memcpy(op, recv_buf + 1, OP_SIZE);
        //memcpy(remain, recv_buf + 1 + OP_SIZE, MAXMSG - 1 - OP_SIZE);
		int n = recvfrom(sockfd, (char *)recv_buf, MAXMSG, MSG_WAITALL, (struct sockaddr *)&recv_addr, &recv_len);
		if (n < 0) {
			sendto(sockfd, (const char *)buffer, len,
				0, (const struct sockaddr *) &servaddr, sizeof(servaddr));
			//n = recvfrom(sockfd, (char *)recv_buf, MAXMSG, MSG_WAITALL, (struct sockaddr *)&recv_addr, &recv_len);
			continue;
		}
		...
		if (seq == ack_num && strncmp(recv_buf + parse_idx, ACK, strlen(ACK)) == 0) {
			waiting = 0;
		}
		/*
        if (seq == noack_num && strcmp(op, "ACK") == 0) {
            waiting = 0; 
        }
		*/
		/***********************************************
...
void* p2p_server(void* arg) {
    ...
    // Set socket options //macos may need to comment the following
	/*
		...
		for(int i = 0; i < fd_count; i++) {
			...
			if (pfds[i].revents & POLLIN) {
				if (pfds[i].fd == server_fd) { /* the server receives a connection request */
					...
					//sleep(2);
					//printf("%ld",file_size);
					while((bytes_read = fread(buffer, sizeof(char),MAXMSG, fp)) > 0) {
...
int p2p_client(unsigned int ip, unsigned short port, char *file_name) {
	...
	//sleep(2);
	while (file_size > 0 && (bytes_read = recv(sock, buffer, MAXMSG, 0)) > 0){
